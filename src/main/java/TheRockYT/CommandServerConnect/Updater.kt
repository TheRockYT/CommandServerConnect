...
class Updater(private val version: String, private val api: String, private val plugin: Plugin) {
    ...
    fun check() {
        ...
        if (oldState != state) {
            CommandServerConnect.broadcast(CommandServerConnect.replacePlaceholder(CommandServerConnect.config?.get("messages.update."+ CommandServerConnect.updater?.state.toString().toLowerCase())), CommandServerConnect.config?.getString("permission.updates"));
        }
    }
